...
| Upload tile from cli        | =omcli upload-product -p </path/my.pivotal>=                                                                             |
| Upload stemcell from cli    | =omcli upload-stemcell -s </path/stemcell.tgz>=                                                                          |
| List products               | =omcli curl -s -p /api/v0/staged/products=                                                                               |
| Get attribute               | =omcli curl -s --path /api/v0/deployed/products/${product_guid}/credentials/.properties.pks_uaa_management_admin_client= |
| Get tile properties         | =bosh deployments=, =omcli curl --path /api/v0/staged/products/${bosh-deployment}/properties -s=                         |
** Ops manager
| Name                | Command                                                                  |
|---------------------+--------------------------------------------------------------------------|
